...
from rl.networks.envs import make_vec_envs_scenic
...
def main():
    ...
    env_config = config = ConfigNoArgs()
    ...
    # scenario_list = [() for i in range(algo_args.num_processes)]
    ...
    for j in range(num_updates):
        ...
        for step in range(algo_args.num_steps):
            ...
            for i, info in enumerate(infos):
                if 'episode' in info.keys():
                    print(f"EPISODE is in info.keys for process {i}")
        ...
        print(f"UPDATE INFO {value_loss, action_loss, dist_entropy}")
